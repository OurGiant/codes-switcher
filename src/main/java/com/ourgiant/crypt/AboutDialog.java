...
import com.ourgiant.crypt.util.NetworkFetchException;
...
final class AboutDialog extends JDialog {
     ...
     * startup check found a newer version) must never block the app — the user can ignore it
     * entirely, keep working, and stay on the current version if they want. Help > About (a
     * deliberate click) stays modal, the normal expectation for that kind of dialog.
     */
    AboutDialog(Frame parent, UpdateChecker.ReleaseInfo knownNewerRelease) {
        super(parent, "About Codes Switcher", knownNewerRelease == null);
    ...
    private static String buildHtml(String currentVersion) {
        return """
            ...
            <h2 style="margin-top: 0;">Codes Switcher</h2>
            <p>Version %s</p>
            <p>A GPG file decryptor and universal text encoder/decoder.</p>
            <p>&copy; OurGiant</p>
            </body></html>
